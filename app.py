...
if all_data is not None:
    ...
    with t2:
        ...
        # 2. VPT: Volume Price Trend (Big Money Indicator)
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.metric("Z-Score", f"{z_score:.2f}", help="Extends: >2.0 (High), <-2.0 (Low)")
            st.metric("VPT Signal", vpt_trend, help="Volume-weighted price trend.")
        with c2:
        ...
        st.markdown(f"""
            <div style="background-color: #1E3A8A; padding: 20px; border-radius: 10px; border-left: 5px solid #3B82F6;">
                ...
                <p style="color: #93C5FD; font-size: 14px; margin: 0;">
                    Price is <b>{z_score:.2f} SD</b> from the mean. Current condition: <b>{"OVEREXTENDED" if abs(z_score) > 2 else "HEALTHY"}</b>.
                </p>
            </div>
