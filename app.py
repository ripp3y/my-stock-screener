...
import streamlit as st # Always at the top to prevent NameErrors
...
for t in all_tickers:
    ...
    if not hist_mo.empty and not hist_5d.empty:
        ...
        # Simpler string for the small font footer
        summary_items.append(f"{t}: {round(abs_perf, 1)}%")

# --- STEP 4: INTERACTIVE SORTABLE DASHBOARD ---
st.subheader("🏆 Strategic Alpha Leaderboard")
df_master = pd.DataFrame(master_list).sort_values(by="Alpha %", ascending=False)

st.dataframe(
    df_master, 
    width="stretch", # Optimized for Chromebook/Mobile
    hide_index=True,
    column_config={
...
# --- STEP 5: CLEAN SMALL FONT FOOTER ---
# "Last Month Performance" anchor for clear time-frame context
st.caption(f"📅 **Last Month Performance:** {' | '.join(summary_items)}")
st.caption(f"⚓ *S&P 500 Benchmark: {round(spy_perf, 2)}%*")
