...
import streamlit as st # Ensures 'st' is defined from the start
...
for t in all_tickers:
    ...
    if not hist_mo.empty and not hist_5d.empty:
        ...
        # Collect data for the small-font footer
        historical_summary.append(f"{t}: {round(abs_perf, 1)}%")

# --- STEP 4: INTERACTIVE SORTABLE DASHBOARD ---
st.subheader("🏆 Strategic Alpha Leaderboard")
df_master = pd.DataFrame(master_list)
df_sorted = df_master.sort_values(by="Alpha %", ascending=False) # Default sort

st.dataframe(
    df_sorted, 
    width="stretch", # Chromebook optimized
    hide_index=True,
    column_config={
...
# --- STEP 5: SMALL FONT FOOTER ---
# Using st.caption for that "small font" look under the chart
st.caption(f"📅 **Last Month Performance:** {' | '.join(historical_summary)}")
