...
# --- 1. SETTINGS & MOBILE LAYOUT FIX ---
st.set_page_config(page_title="Alpha Scout", layout="wide")

# This CSS forces the price and performance % to stay on the same horizontal line
st.markdown("""
    <style>
...
# PBRA fully removed to prevent download failures
team_intel = {
...
if all_data is not None:
    ...
    for t in tickers:
        try:
            ...
            # Relative Strength Score vs S&P 500
    ...
    # Sorting Leaderboard by RS score
    sorted_stats = sorted(stats, key=lambda x: x['rs'], reverse=True)
    
    # 2-Column Grid for scannability on phone screens
    ...
    with t2:
        ...
        # Using metrics for risk visibility
    ...
    with t3:
        ...
        if not insider_df.empty:
            # Verified and closed all brackets to fix the SyntaxError
    ...
    st.error("📡 Sync Issue: Verify connection or API key.")
